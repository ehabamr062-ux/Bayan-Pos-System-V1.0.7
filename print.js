            ...
            <script>
                window.onload = function() {
                    if (typeof QRCode !== 'undefined') {
                        const qrElements = document.querySelectorAll('.invoice-qr-code');
                        qrElements.forEach(el => {
                            new QRCode(el, {
                                text: decodeURIComponent(el.getAttribute('data-qr')),
                                width: 120,
                                height: 120,
                                colorDark : "#000000",
                                colorLight : "#ffffff",
                    ...
                    setTimeout(function() {
                        window.focus();
                        window.print();
                        // تم إلغاء الإغلاق التلقائي بناء على طلب المستخدم لضمان عدم اختفاء نافذة المعاينة
                    }, 500);
                };
            </script>
        </body>
...
function build80mmStandard(d) {
    ...
    // إنشاء الباركود QR Code محلياً أوفلاين يحتوي على تفاصيل الفاتورة
    const qrData = `Inv: #${d.invoiceNumber}\nType: ${d.invoiceType}\nTotal: ${parseFloat(d.totalAmount).toFixed(2)}`;
    const qrHtml = `<div class="invoice-qr-code" data-qr="${encodeURIComponent(qrData)}" style="margin:0 auto; width:120px; height:120px; padding:2px; background:#fff;"></div>`;
