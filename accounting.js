...
if (finalData.length === 0) tbody.innerHTML = '<tr><td colspan="13" style="text-align:center; padding:20px;">لا توجد بيانات تطابق البحث</td></tr>';
        ...
        function viewInvoiceItems(invoiceId, type = '', autoPrint = false) {
            ...
            if (type) {
                ...
                if (typeClean.includes('بيع') || typeClean.includes('مبيعات')) category = 'sales';
            ...
            // فلترة الحركات التي تطابق رقم الفاتورة والنوع/الفئة
            let invoiceItems = transactions.filter(t => {
                if (String(t.invoiceId) !== String(invoiceId)) return false;
                if (category) {
                    if (category === 'sales') return t.type && (t.type.includes('بيع') || t.type.includes('مبيعات'));
                    if (category === 'purchase') return t.type && (t.type.includes('شراء') || t.type.includes('مشتريات'));
                    if (category === 'receipt') return t.type && t.type.includes('قبض');
                    if (category === 'disbursement') return t.type && t.type.includes('صرف');
                    if (category === 'adjustment') return t.type && t.type.includes('تسوية');
                    if (category === 'transfer') return t.type && t.type.includes('تحويل');
                }
                return true;
            });
            
            // إذا كانت الفاتورة تجارية (بيع أو شراء)، نستبعد حركات المقبوضات والمدفوعات الفردية المرتبطة بها من جدول الأصناف
            if (category === 'sales' || category === 'purchase') {
            ...
            // أخذ كافة العناصر بدون استثناء لتجنب اختفاء أي صنف
            let rawItems = [...invoiceItems];
            
            // تم حذف الشرط السابق الذي كان يستبعد (isInvoiceHead) لأنه كان يحذف الصنف الأول دائماً
        ...
        async function saveReceipt(closeAfterExplicit = false) {
            ...
            const isCloseChecked = document.getElementById('receiptCloseCheck')?.checked || false;
            ...
            if (isCloseChecked) {
                setTimeout(() => {
                    ...
                    else if (typeof switchSection === 'function') switchSection('dashboard');
                }, 600);
            }
        ...
        async function saveDisbursement(closeAfterExplicit = false) {
            ...
            const isCloseChecked = document.getElementById('disburseCloseCheck')?.checked || false;
        ...
        function generateAccountStatement(accountId) {
            ...
            if (!targetId) {
                document.getElementById('statementTableBody').innerHTML = '<tr><td colspan="9" style="text-align:center;">الرجاء اختيار الحساب أولاً لعرض التقرير</td></tr>';
        ...
        window.loadStatementDetails = function(invoiceId, type, rowElement) {
            ...
            if (type && (type.includes('قبض') || type.includes('صرف'))) {
                document.getElementById('statementDetailsBody').innerHTML = '<tr><td colspan="8" style="padding: 40px; color: #94a3b8; font-size: 1.1rem; text-align: center;">لا توجد تفاصيل أصناف (حركة مالية)</td></tr>';
                return;
            }
            
            if (!invoiceId) {
                document.getElementById('statementDetailsBody').innerHTML = '<tr><td colspan="8" style="padding: 20px; text-align: center;">لا توجد تفاصيل أصناف لهذه الحركة</td></tr>';
                return;
            }
            ...
            if (invoiceItems.length === 0) {
                document.getElementById('statementDetailsBody').innerHTML = '<tr><td colspan="7" style="padding: 20px; text-align: center;">لا توجد تفاصيل أصناف (حركة مالية أو قيد)</td></tr>';
                return;
            }
            ...
            invoiceItems.forEach(i => {
                ...
                // محاولة جلب الصنف من الداتا بيز للحصول على تصنيف إن وجد
                const productDbInfo = productsDB.find(p => p.name === i.product);
                const category = productDbInfo ? productDbInfo.category : '-';
                
                html += `
                    <tr>
                        <td>${productDbInfo ? productDbInfo.id : '-'}</td>
                        <td style="font-weight:bold;">${i.product || '-'}</td>
                        <td>${i.unit || '-'}</td>
                        <td>${qty}</td>
                        <td>${price.toFixed(2)}</td>
                        <td>0.00</td> <!-- Discount could be added if available -->
                        <td style="font-weight:bold; color:var(--main-blue);">${total.toFixed(2)}</td>
                        <td>${category}</td>
                    </tr>
                `;
            });
        ...
        window.handleStmtAction = function(action) {
            if (!window.currentStmtInvoiceId || !window.currentStmtType) return;
            ...
            if (action === 'print') {
                ...
                document.getElementById('stmtConfirmDeleteBtn').onclick = () => {
                    ...
                    setTimeout(() => generateAccountStatement(), 1000);
                };
            }
        };
